...
We want to combine the risk ratio for gun ownership with the results by [@kellermann1993gun] who found that a history of domestic physical fights increases the risk of dying by homicide in the home by 4.4 times. If one does both, we would want our model to say that the risk is even higher than either, but how much exactly? In this example, and in many others, we have decided that we multiply the risk ratios. That is

* If you both live in a household with physical violent fights and gun(s), you risk of dying from homicide in the home is $2.0\cdot 4.4=8.8$ times higher than if you did neither.
...
* The authors in [@anglemyer2014accessability] must have thought about and tested the appropriateness of combining studies that adjust for physical violence with those that don't. Since they ended up doing so, they have probably found that it is not a big violation.
...
To compute the normalizing factor we need to know the proportion of Americans who belong to each combination of gun ownership and physical violence. For example we need the proportion of the American population who lives in a household where there is a gun and a history of physical violence. Recall that the proportion of households that owns a gun ($40\%$) and the proportion that has a history of violence ($5.7\%$). If the gun owning and non gun owning households were independent, they would be equally likely to be violent and so we could compute the joint proportion as $40\%\cdot 5.7\%=2.3\%$. It doesn't sound outrageous that they should be independent and since we are unlikely to find the actual proportions, we assume that they are independent. Then the normalizing factor is
$$
...
$$
The potential caveat to assuming that they are independent is that we get a more imprecise number for the normalizing factor. On the other hand, the normalizing factor is already an imprecise number because the odds ratios 4.4 and 2.0 are uncertain estimates. The confidence intervals for 4.4 is 

We will often 


It is very probable that there exists research that could guide us towards a better guess for the proportions, so we don't have to assume that gun ownership and physical violence are independent. However, duean actual answer to
Our intuition tells us that it is probably not far from  if they arecould imagine that violent households tend to be poorer and therefore  , we can  Let $P(\text{gun, violence})$, $P(\text{no gun, violence})$, $P(\text{gun, no violence})$ and be 

* 


The normalizing factor is the average risk ratio, so if we take the example with We have to compute a new normalizing factor for the joint risk ratio. The normalizing factor is the average rate of 
