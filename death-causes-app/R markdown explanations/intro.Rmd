...
$$0.005\%\cdot 50\%+50\% \cdot 0.005\%\cdot \frac{1.9}{1.9\cdot 40 \%+60\%}$$
...
[synes vi skal have en mindre overskrift her. det virker som om det er nyt afsnit og ikke en underdel til det foregående]
## Normalization factor
Looking at the more complicated part of the above equation, we are left with
$$0.0025\%\cdot \frac{1.9}{1.9\cdot 40 \%+60\%} $$
...
$$
x\cdot 60\%+x\cdot 1.9\cdot 40\%=0.0025\%
$$
$$
x \cdot (60\%+ 1.9\cdot 40\%)=0.0025\%
$$
$$
x=0.0025\%\frac{1}{60\%+ 1.9\cdot 40\%}
$$
...
$$
x\cdot 1.9=0.0025\%\frac{1.9}{60\%+ 1.9\cdot 40\%}
$$
We have hereby shown the formulas.
</details>
$~$  
[synes også vi skal have 60% * 1.0 i brøken for klarhedsskyldt]
...
This curve is computed as a trade-off between being smooth and reflecting the original data. We have decided that the above curve could not exceed 1, because there is no indication that extreme coffee consumption leads to more Alzheimer's than total coffee abstinence. [det er uklart hvorfor vi mener RR ikke kan overstige 1, når nu vores bins topper ved ca. 0.60-0.65 eksempelvis]
...
* The authors in @anglemyer2014accessability must have thought about or tested the appropriateness of combining studies that adjust for physical violence with those that don't. Since they ended up doing so, they have probably found that it is not a giant violation. [skriver de ikke noget om det i artiklen? Eller skal vi bare antage at de har styr på det?]
...
The potential caveat to assuming that they are independent is that we get a more imprecise estimate of the normalizing factor. On one hand the normalizing factor and in particular, the normalized risk are already uncertain estimates. If we consider the homicide example, the normalizing factor when assuming independence was $1.67$. If we didn't assume independence, the normalizing factor could still not be outside the interval $(1.40,1.79)$ [er det udregnet? Og i gvet fald hvordan?] (under the assumption that $5.7\%$ are violent and $40\%$ own guns). For comparison the confidence interval of the 4.4-odds ratio of physical violence is $(2.2,8.8)$ [er det fra artiklen, skal vi have en reference på?] and the confidence interval of the 2.0-odds ratio of gun ownership is $(1.56,3.02)$ [se foregående]. The error from incorrectly assuming independence is therefore not the biggest source of error in this case. In general the independence assumption will matter less when the risk factors are rare or when the risk ratios are similar.  
...
### Colon Cancer
[skal vi sige noget om hvorfor det lige er denne death cause vi gennemgår? Det er ikke helt klart hvorfor denne er speciel, eller er det bare et eksempel]
