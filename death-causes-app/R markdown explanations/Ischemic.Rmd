...
Below is the risk ratios for time since one stopped smoking (SmokeSinceStop) and the average amount smoked (number of cigarette, SmokeTypicalAmount). It is not age adjusted, so it is only for 65-year olds.
...
If you drink a moderate amount of alcohol, you decrease the risk of dying from coronary heart disease, but it can this benefit can be erased from binge drinking (which we call MaxDrinking). We use the numbers from @roerecke2014alcohol, who have made a meta-analysis dose-response curve for alcohol consumption and ischemic heart disease mortalitiy, which is ideal for us. They also show that heavy binge drinking can eliminate the benefit for people with an average alcohol consumption of 1 unit per day. To quantify the binge drinking we use the numbers from @malyutina2002relation, which is one of the studies used by the study of @roerecke2014alcohol. Because the study from @malyutina2002relation is not a meta-analysis, it has a higher variance and therefore we see that drink 5.7 to 8.6 as a max actually has a higher risk ratio than those who binge drink more heavily. This artifact is smoothed out by our interpolation method. 
