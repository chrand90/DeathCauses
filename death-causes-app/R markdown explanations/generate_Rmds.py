...
with open("../../Database/ICD dictionary/icd_dictionary.json", "r") as f:
    ICD_definitions = json.loads(f.read())
with open("resources/references.bib", "r") as f:
    references=[]
...
def has_custom_texts(node_name):
    return node_name in texts
...
def make_opener(title, f, fully_autogenerated=True, insert_metadata=""):
    f.write('---\n')
    f.write('title: "' + title + '"\n')
    f.write('output:\n\
    html_document:\n\
...
def make_optimizability_section(node_name, title, f):
    ...
    f.write("\n\n### Optimizability\n\n")
    f.write("[Optimizability](/model/optimizabilities) is a subjective measure used to describe how easy it is to optimize a certain factor of your life. ")
...
def make_interaction_description(node_name, title, rfgs, f):
    ...
    if text_available:
        ...
        f.write("The combined risk ratio of all risk factors is computed using the [formula](/model/intro#interaction-formulas)\n\n")
...
def fill_out_death_cause_category(node_name, title, f):
    f.write(
        title + " is a **death cause category**. It means that it is a collection of **death causes** and so one can compute the probability of dying from this death cause category by adding up all of its death causes. \n")
    add_text_if_there(f, node_name, "introduction", "after")
    alternative_names(node_name, f)
...
def createOptimizabilities():
    with open(os.path.join("resources", "optimizabilities.Rmd"), "w") as f:
        make_opener("Optimizabilities",f, not has_custom_texts("optimizabilities"))
...
def createReferences():
    with open(os.path.join("resources","references.Rmd"), "w") as f:
        make_opener("References", f, True, 'nocite: |\n\
   '+', '.join(map(lambda x: "@"+x, references))+'\n')
