...
If there are two different risk factors things get more complicated. If we go back to the example about homicide in the home, gun ownership and domestic violence, the un-decomposed probabilities would look like this
...
So how do we actually compute the decomposed probability? Gun ownership centered or domestic violence centered? And how do we handle this in general? We make the decomposition centered around the factors that are easier to optimize. For example in skin cancer, there are two main factors; your race and your sun exposure. It is clearly not possible to change your race, but you can change your (future) sun exposure. Therefore we decompose skin cancer centered on sun exposure. For domestic violence and gun ownership, it is a bit harder to judge which is easier to optimize. Both can be changed by moving which, in theory, can be done tomorrow. They are tied in our optimizability ranking, and we therefore use another decomposition strategy.
...
Though it rarely happens, be aware that the "Extra part" can be negative. It may occur when two factors are similar or measuring the same effect. In this case the negative "Extra part" is subtracted equally among the two factors.

#### Decomposition of $n$ factors

Decomposition of a risk with $n$ risk factors is not very different from decomposition of a risk with 2 risk factors. Instead of only centering on the factor with highest optimizability, we make scheme where we consecutively center on the with second-lowest optimizability, third-lowest optimizability and so on until the highest optimizable factor. Ties in optimizability can also be resolved in a similar way (see below).

<details>
Let $F_1, \dots, F_n$ be the $n$ factors. Let $\psi_1, \dots, \psi_n$ be the user's $n$ values on the $n$ factors. Now let $U(I)$, $I\subseteq \{1, \dots, n\}$ be the smallest risk possible that satisfy
...
$$
S(\emptyset)=U(\emptyset)\\
S(I)=U(I)-\sum_{J\subset I, J\neq I} S(J), \quad \text{for } |I|>0
$$

For $I=\{i\}$, we can interpret $S(I)$ as the pure contribution of factor $i$. For $|I|=2$, $S(I)$ can be interpreted as the "extra part" discussed in the previous section. For $|I|>2$ the interpretation is more complex but it will still some "extra part" to make the sum of all the previous factors add up. 
...
Then, we can write the decomposition as follows

$$
V(i)=\sum_{I\subseteq J_1, i\in I} \frac{S(I)}{|I|} \quad \text{for } i\in J_1\\
$$
