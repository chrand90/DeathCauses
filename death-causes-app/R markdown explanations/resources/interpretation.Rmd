...
The number of years lost to a cause *because of a risk factor* has to be interpreted in a way that be unexpected. One might think that the number should be the increase in life expectancy if the user obtained the [optimal value](#optimal-value) of the risk factor, but is not how we compute it. Instead, the number of years lost to a cause because of a risk factor is the increase in life expectancy if the user avoids having that risk factor as the [best explanation](#best-explanation) of the death cause. One way to avoid that a risk factor is the best explanation of one's death is, of course, to have the optimal value of that risk factor. However, the optimal value would also prevent deaths where the risk factor was one of the true explanations, but not *the* best explanation. 

The reason why we focus on the life expectancy increase when avoiding the risk factor as the best explanation is that it is [easier to plot](/model/notwritten).

### Multiple factors
Sometimes the program will say that a user loses lifetime to *multiple factors*. To see this, imagine that for lung cancer, you lose 1 month to smoking and 1 month to [unknown risk factors](#unexplained). One might think that you would then lose 2 months to both risk factors as a whole, but you actually lose a little bit more than 2 months. The reason is that when you avoid the deaths of say, smoking, there is one less reason for you to die. Death is unavoidable, so this means that all the other reasons, such as dying from lung cancer because of unknown factors, becomes slightly more likely. When dying from lung cancer from unknown factors is more likely, the significance of avoiding it becomes greater and so the increase increase in life expectancy would be more than 1 month. To account for this extra risk, we put it into the category labeled "multiple factors".
