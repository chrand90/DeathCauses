...
setGeneric(name="makePlot",
           def=function(x, plotFrequencies)
           {
...
setMethod("makePlot",
          signature=c(x="RawRiskRatioTable", plotFrequencies="logical"),
            ...
            if(n==1){
              ...
              if(plotFrequencies){
                p <- ggplot2::ggplot(df, ggplot2::aes(x=x+width/2,y=freq, width=width*0.95, fill=freq))+
                  ggplot2::geom_bar(stat='identity', orientation="x")+
                  ggplot2::scale_x_continuous(breaks=dc@tickPositions, labels = dc@tickLabels)+
                  ggplot2::scale_fill_gradient(low = "green", high = "black", na.value = NA)+
...
setMethod("makePlot",
          signature=c(x="RiskRatioTable", plotFrequencies="logical"),
          function(x, plotFrequencies=F){
...
setMethod("makePlot",
          signature=c(x="RiskFactorGroup", plotFrequencies="logical"),
          function(x,plotFrequencies=F){
