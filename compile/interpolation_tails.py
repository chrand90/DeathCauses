...
class Formula:
    ...
    def packaged_call(self, xval_as_list):
        print("xval as list", xval_as_list)
    ...
    def find_roots(self):
        ...
        if not (isinstance(a, float) or isinstance(a, int)):  # this means that the coefficient is itself a formula
            return {'diskriminant': [[str(a), str(b), str(c)]]}
        if abs(a) < 1e-8:
            if abs(b) < 1e-8:
    ...
    def evaluate_candidate_points(self, points):
        ...
        for critical_point in points:
            if self.point_within_bounds(critical_point):
                ...
                candidates.append((y, critical_point))
        return candidates
    ...
    def evaluate_first_bounds(self):
        ...
        first_bound_vals.append((lower_bound_value, lower_bound_argument[x0]))
        upper_bound_argument = {x0: self.bounds[0][1]}
        upper_bound_value = self.__call__(upper_bound_argument)
        first_bound_vals.append((upper_bound_value, upper_bound_argument[x0]))
        ...
        for index_of_not_fixed in range(len(self.xvars)):
            ...
            for chosen_bounds in product(*bounds_options):
                if all(bound is not None for bound in chosen_bounds):
                    ...
                    value_dic.update({fixed_variable: min_point[1]})
        ...
        for index_of_not_fixed in range(len(self.xvars)):
            ...
            for chosen_bounds in product(*bounds_options):
                if all(bound is not None for bound in chosen_bounds):
                    value_dic = {var: bound for var, bound in zip(non_fixed_variables, chosen_bounds)}
                    new_f = self.insertValues(value_dic)
                    discriminant = new_f.diff(fixed_variable).find_roots()
                    candidates.append({"values": value_dic, "discriminant": discriminant['diskriminant']})
    ...
    def get_mins_for_all_fixed_others(self):
...
if __name__ == '__main__':
    ...
    nontrivial = "0+0.3*x0^3+0.3*x0^1*x1^2+0.04*x0^3*x1^3+-4*x0^1+-1*x1^1+0.4*x2^2+0.4*x0^1*x2^3"
    f = Formula(nontrivial, bounds=[(0, 6), (0, 4), (0,3)])
    print(str(f))
    print(f.diff('x0'))
    print(f.diff('x1'))
    # print(f.find_min())
