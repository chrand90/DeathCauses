...
def make_simultane_and_get_writtenF(RRs,interaction_function):
    ...
    reducedFactorNames=remove_duplicates([item for sublist in factorNamesCombined for item in sublist]) #list of all factor names
...
def thin_out_big_risk_ratio(list_of_lists_of_factor_levels,
    ...
    name_to_level={name:level for name,level in zip(flat_factor_names, flat_factor_levels) if name not in list_of_duplicates}
