...
#             print a,b,c
            new_rows[tuple(factors)]=new_rows.get(tuple(factors),0)+row[-1]*intersect/category_size([age_interval])
...
def make_simultane(listOfdataframes):
    ...
    for (df, _) in sorted_dfs:
        ...
        for row in dfrows:
            ...
            reverse_name = {factor:index for index, factor in enumerate(factorDicKeys) if factor in df.get_FactorNames()}
            reverse_nameKeys=reverse_name.keys()
            reverse_nameValues=reverse_name.values()
            possible_factor_values = {factor:[row[index]] for factor, index in zip(reverse_nameKeys, reverse_nameValues)}
